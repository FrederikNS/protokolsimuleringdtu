 ...
 *
 */
public class SensorRemake extends Location implements Transmitter, Prepareable, Comparable<Sensor>, NoteConstants, DataConstants, EndSteppable {

	public static final int INVALID_SENSOR_ID = -1;
	public static final int ALL_SENSORS = -2;
	
	protected static final int OPTION_SEND 	 	= 0x00000001;
	protected static final int OPTION_RECEIVE 	= 0x00000002;
	...
	public SensorRemake(Location loc) {
		super(loc);
	 ...
	 * Handles id-look-up
	 */
	private static Hashtable<Integer,SensorRemake> idToSensor = new Hashtable<Integer,SensorRemake>();
	 ...
	 * @throws LabelNotRecognizedException If no sensor has been assigned that label.
	 */
	public SensorRemake getSensor(String label) throws LabelNotRecognizedException {
	...
	public boolean assignLabel(String label) {
		...
		if(label == null || label.trim().equals("")) {
			...
			toReturn = labelToID.put(label, id) != null;
		}
		sensorLabel = label;
		return toReturn;
	}
		...
		if(ingoing != null) {
			...
			case TYPE_RECEIVED_SUCCESSFULLY:
				...
				if(isForMe(ingoing)) {
					...
					outgoing = null;
				} else if(waiting && ingoing.sender == outgoing.receiver) {
					waiting = false;
					//add random delay?
				}
				break;
			case TYPE_RECEIVED_UNSUCCESSFULLY:
				...
				if(outgoing != null && outgoing.receiver == ingoing.sender) {
